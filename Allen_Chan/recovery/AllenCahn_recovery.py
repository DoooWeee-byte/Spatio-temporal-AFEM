...
while True:
    ...
    print("err/norm:", err/uh_grad_norm)
...
while t1 <= T:
    ...
    eta0 = space.recovery_estimate(uh00,'area_harmonic')
    ...
    #画数值解图像
    if (abs(t1 - 1e-4) < 1e-5) | (abs(t1 - 0.049499999999999975) < 1e-5 ) | (abs(t1 -0.20150000000000004) < 1e-5) | (abs(t1 - 0.30069999999999997) < 1e-5) \
            | (abs(t1 - 0.4030999999999993) < 1e-4) | (abs(t1 - T) < 1e-4):
        fig = plt.figure()
...
plt.ylabel("error estimate",fontsize=16)
