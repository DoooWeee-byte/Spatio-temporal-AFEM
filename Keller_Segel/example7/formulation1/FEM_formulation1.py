...
from Keller_Segel_equation_2d import KSData,KSData1,KSData2,KSData3
...
for k in range(0, maxit):
    ...
    for i in range(nt):
        ...
        while True:
            ...
            AU = M + dt*L
            vh_temp = space.function()
            #vh_temp[:] = uh_old[:]/(1 + vh_old[:])
            K,K1 = space.ks_nonlinear_matrix_iter(uh_old)
            K += K1
            vh_temp[:] *= -1
            bU = dt*K@vh_new[:] + M@uh0[:] + dt*FU
            '''
            space.set_neumann_bc_u(F=bU, gN=neumann, u=vh_temp)
            AU, bU =bc.apply(bU, A = AU)
            '''
            uh_new[:] = spsolve(AU, bU)
            if(np.linalg.norm(uh_new - uh_old) < 1e-12) & (np.linalg.norm(vh_new - vh_old) < 1e-12):
                break
            uh_old[:] = uh_new[:]
            vh_old[:] = vh_new[:]
