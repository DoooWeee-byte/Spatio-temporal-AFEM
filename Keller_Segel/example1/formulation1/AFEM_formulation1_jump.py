...
from fealpy.functionspace.LagrangeFiniteElementSpace_test2 import LagrangeFiniteElementSpace
...
while True:
    ...
    etaU = space.jump_estimate(uh0)
    etaV = space.jump_estimate(vh0)
...
for k in range(nt):
    ...
    while True:
        ...
        #计算误差估计子
        etaU = space.jump_estimate(uh1)
        etaV = space.jump_estimate(vh1)
        errU = np.sqrt(np.sum(etaU**2))
        errV = np.sqrt(np.sum(etaV**2))
        ...
        print("ErrorEstimateU", errU)
        print("ErrorEstimateV", errV)
