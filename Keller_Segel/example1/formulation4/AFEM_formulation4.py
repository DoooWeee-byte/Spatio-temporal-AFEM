...
while True:
    ...
    plt.close()
    j += 1
    if (errU/uh_grad_norm < tol) |  (space.number_of_global_dofs() > 5000):
        break
...
for k in range(nt):
    ...
    while True:
        ...
        while True:
            ...
            bU = M@uh0[:] - 0.5*dt*L@uh0[:] + 0.5*dt*(K2 + D)@uh0

            uh_new[:] = spsolve(AU, bU)
            print("errorU",np.linalg.norm(uh_new - uh_old))
            print("errorV",np.linalg.norm(vh_new - vh_old))
            count += 1
            if(np.linalg.norm(uh_new - uh_old) < 1e-9) & (np.linalg.norm(vh_new - vh_old) < 1e-9):
                break
            if count > 200:
                break
            uh_old[:] = uh_new[:]
        ...
        uh1[:] = uh_new[:]
        vh1[:] = vh_new[:]
        error_uh = np.linalg.norm(uh_new - uh_old)
        ...
        if (errU/uh_grad_norm < tol1) & (errV/vh_grad_norm < tol1):
            ...
            print("less")
            break
        #elif  (space.number_of_global_dofs() > 5000):
        #    break
        else:
            #标记
            isMarkedCellU = smesh.refine_marker(etaU, theta, method='MAX')
