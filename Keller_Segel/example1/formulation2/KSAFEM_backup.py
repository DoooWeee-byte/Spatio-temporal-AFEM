...
for k in range(nt):
    ...
    ctheta = args.ctheta
    while True:
        mesh0 = copy.deepcopy(smesh)
        #网格粗化
        isMarkedCellU = mesh0.refine_marker(etaU, ctheta, 'COARSEN')
        isMarkedCellV = mesh0.refine_marker(etaV, ctheta, 'COARSEN')
        isMarkedCell = isMarkedCellU & isMarkedCellV
        mesh0.coarsen_triangle_rg(isMarkedCell)

        #计算误差(细 到 粗 到 细)
        spaceH = LagrangeFiniteElementSpace(mesh0, p=degree)
        uh0 = spaceH.function()
        vh0 = spaceH.function()
        retain = mesh0.retainnode
        uh0[:] = uh1[retain] # 细到粗, uh0是粗空间上的解
        vh0[:] = vh1[retain]

        halfedge = mesh0.entity("halfedge")
        rnidx = mesh0.retainnode #粗网格点的原始编号
        dnidx = mesh0.deletenode #细网格中被删除的点的编号(在细网格中的编号)
        dn2e = mesh0.deletnode2edge
        uh00[rnidx] = uh0[:]
        uh00[dnidx] = 0.5*(uh0[halfedge[dn2e, 0]] + uh0[halfedge[halfedge[dn2e, 3], 0]])
        
        vh00[rnidx] = vh0[:]
        vh00[dnidx] = 0.5*(vh0[halfedge[dn2e, 0]] + vh0[halfedge[halfedge[dn2e, 3], 0]])
        
        # 计算 || uh1 - uh00 ||^2_{L2}  和  || grad(  uh1 - uh00 ) ||^2_{L2}
        uh00[:] = uh1 - uh00
        measure = smesh.entity_measure('cell')
        qf = space.integrator
        bcs, ws = qf.get_quadrature_points_and_weights()
        uvalue = uh00.value(bcs) #(NQ, NC)
        val = np.einsum('q, qc, qc,  c -> c', ws, uvalue, uvalue,  measure)
        valL2 = np.sum(val)

        grad = uh00.grad_value(bcs)**2
        grad = np.sum(grad, axis=2)
        gradL2 = np.einsum('q, qc, c->', ws, grad, measure)

        eta_coarseU = valL2/dt + gradL2

        # 计算 || vh1 - vh00 ||^2_{L2}  和  || grad(  vh1 - vh00 ) ||^2_{L2}
        vh00[:] = vh1 - vh00
        measure = smesh.entity_measure('cell')
        qf = space.integrator
        bcs, ws = qf.get_quadrature_points_and_weights()
        vvalue = vh00.value(bcs) #(NQ, NC)
        val = np.einsum('q, qc, qc,  c -> c', ws, vvalue, vvalue,  measure)
        valL2 = np.sum(val)

        grad = vh00.grad_value(bcs)**2
        grad = np.sum(grad, axis=2)
        gradL2 = np.einsum('q, qc, c->', ws, grad, measure)

        eta_coarseV = valL2/dt + gradL2

        if (eta_coarseV > TOLcoarse/T) | (eta_coarseU > TOLcoarse/T) | (ctheta >= 1):
            if ctheta >= 1:
                ctheta -= 0.1
            break
        else:
